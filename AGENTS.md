...
当前成员 D 负责前端与 API 交付板块，具体职责以 `分工.md`、`doc/` 和已冻结的接口契约为准：

- 维护 `app/api` 与 `frontend/` 范围内的代码。
- 按 `doc/api.md` 实现 REST API，并对接 Workflow、会话和消息状态。
- 后续开发 Web 控制台，展示 Agent 团队状态、任务历史、工具调用链路、Token 统计和会话管理。
- 配合端到端联调、演示录制和部署文档整理。

成员 D 还必须遵守以下协作要求：

1. **文档是唯一事实依据。** 一切实现以 `doc/`、`分工.md` 和已冻结的接口契约为准，不自由增加未约定的能力、字段或流程。
2. **发现文档错误先改文档。** 先修改对应文档，再修改代码；文档发生修改后，必须通知其他成员，并确保相关分支同步最新内容。
3. **写代码前先更新项目。** 开始编码前检查工作区状态，获取远端最新提交并同步当前基线；不得覆盖或回退其他成员已有改动。
4. **编码必须使用新分支。** 不直接在主分支开发。分支应从最新基线创建，使用清晰的成员 D 前缀，例如 `member-d/<topic>`，且不使用 `codex/` 前缀。
5. **提交信息必须规范且使用中文。** 提交按逻辑单元拆分，采用 `类型（范围）：说明` 格式，例如 `新增（API）：实现会话创建接口`、`修复（前端）：修正任务状态展示`。
6. **修改范围保持清晰。** 默认只修改成员 D 负责的 API、前端和明确关联的文档；需要修改其他成员负责目录时，先沟通并说明原因。
7. **提交前必须验证。** 至少运行与改动相关的测试，并检查 `git diff`、`git status --short`，确认没有误提交删除文件或无关改动。
8. **接口变更必须同步。** API 路径、请求/响应字段、状态码和异步执行语义发生变化时，先同步更新 `doc/api.md`，再同步代码和测试，并通知接口消费者。
