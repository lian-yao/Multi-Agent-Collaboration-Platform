...
export function buildCollaboration({
  ...
  const nodes: CollabNode[] = entries.map((entry) => {
    ...
    // 拿它回答旧对话就是在编造（见 `runModelFor`）。平台节点跑平台自己的配置，不取。
    const modelAtRun = isWorker
      ? runModelFor(metrics, metricStageOf(entry.id, plan.length > 0 || flow.length > 0))
      : "";
    return {
      id: entry.id,
      kind: entry.kind,
      ...
      modelAtRun,
      agentKnown: isWorker ? Boolean(agent) : false,
      provider: isWorker ? agent?.provider_name ?? agent?.provider ?? "" : "",
      params: isWorker ? paramsOf(agent, modelAtRun) : [],
