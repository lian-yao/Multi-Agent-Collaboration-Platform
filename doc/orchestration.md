...
**角色的 system prompt 不再假设固定三步**（ADR-006 修订 2026-09-24）：同一角色可能在一次执行里
并行出现多次、上游可能是零个或多个子任务、面向使用者的最终交付物由合成器产出。因此三段提示词
统一按「本次执行里的一个步骤」写，并显式说明「并行子任务互不可见」与「产出会被下游或合成器合并」。
`reporter` 还会按输入判断本次是「子任务成稿」还是「单 Agent 直答」，据此决定口吻。
