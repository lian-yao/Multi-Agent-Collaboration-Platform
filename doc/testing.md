...
**第十四轮（同日）：问题改写前置步骤 + 角色提示词与软件对齐**。使用者要求两条：
「增加一步问题改写，把用户的输入，通过会话上下文，改写的更完整」与「修改一下提示词，
每个 agent 的提示词，使其更符合现在的软件」。设计见
[ADR-037](decisions/037-query-rewrite-and-prompt-alignment.md)。

**改写**做成一次执行的**前置步骤**（静态在收集前、动态在规划前，两条链路共用
`app/orchestration/rewrite.py` 与同一个 `rewrite_activity`）：输入是本轮原文 + 会话历史 +
长期记忆 + 本轮附件**文件名**，输出是一段自包含任务。**不做成第 4 个 `PipelineStage`**——
那个三元组同时撑起 Dapr 子工作流 ID、阶段状态键、stage-trace 出参与前端阶段列表，而改写没有
角色、不产出交付物。结果随 `workflow_runs.checkpoint` 落库（`rewritten_task` /
`rewrite_source`），`doc/orchestration.md` 与 `doc/api.md` 同步。

**提示词**按「位置 → 手上真有什么 → 边界与纪律」重写三个角色，并同步规划 Agent：
写清会话工作区文件工具（含具体工具名）、附件、MCP 与网页搜索、**覆盖/删除是人工审批不是失败**、
路径越界会被服务端拒绝、下游只看得到你写出来的内容。

真机验收时撞到一个**由本轮改动引入的真事故**，值得记下来：

1. **首个活动就炸**：改写节点用 `get_settings()`（**裸环境变量**）解析模型，而使用者的模型是
   在「工具与配置」里配的、经**数据库**生效——静态/动态的其它节点走 `resolve_agent_settings()`
   所以一直正常，唯独新节点看不见那份配置，于是在**执行的第一步**报"模型缺失"、整条执行失败。
   修法：与规划节点共用同一个平台节点 id（`PLANNER_AGENT_ID`）解析模型。
2. **降级保护漏了"构造模型"这一步**：`build_chat_model()` 写在 try 之外，模型构造失败直接
   冒到工作流层。改写是"增强而非必需"，**构造与调用都必须在保护里**——修完补了专门的回归用例
   （用 provider=openai 但空模型名构造，断言退回原文）。

验证：

- 单测：`test_query_rewrite.py` **+10**（上下文段落顺序、空段不产生、模型失败/空输出/超长/
  原样抄回/**构造失败** 全部退回原文、空输入不花模型调用）；`test_agent_roles.py` +1
  （三段提示词必须提到工作区、文件工具与审批，缺一个就红）；`test_dynamic_pipeline.py` +1
  （规划提示词说明角色实际能力与"任务已被改写补全"）；
  `test_workflow_pipeline.py` / `test_workflow_dynamic.py` / `test_pipeline_state.py` 按新契约
  更新（**第一个活动必须是 `rewrite_activity`**、checkpoint 多出两个字段）；
  全量 `pytest tests/unit tests/integration/test_api.py tests/integration/test_workspace_api.py`
  → **1012 passed / 12 failed**（12 条仍是缺 `pypdfium2` 的 PDF 用例）；
- **真机**（本地服务形态，动态 + 静态各一条）：

  | 轮次 | 用户原文 | 改写后（`rewrite_source=model`） |
  | --- | --- | --- |
  | 动态 第 1 轮 | 帮我给团队写一份本周周报的提纲，重点写清楚每个人的进展与风险 | 请为团队撰写一份本周周报的提纲，要求覆盖每位成员的进展与风险，并把这两部分作为重点写清楚。 |
  | 动态 第 2 轮 | **再详细一点** | 把上一轮那份「团队周报（骨架版）」做得再详细一些：在保留其性质声明和全部【待补】标注的前提下，把骨架进一步展开细化——例如补充/深化逐成员进展与风险阻塞表格的字段说明、填写规则、最小回填清单及风险与建议部分……约束不变：不编造团队名称、成员名单、进展与风险等任何实质内容。 |
  | 静态 第 3 轮 | 把刚才那份提纲里的风险部分再具体一点 | 把上一轮生成的《团队周报（骨架 v1 · 内容待补）》提纲中的「风险与阻塞」…… |
...
**补记（2026-09-24，合并 master 之后）**：合并带进来的新颜色（协作画布的平台节点与
「部分完成」横幅 7 处、config 页记忆条目 2 处）当时还是字面量，暗色下会保持浅色取值。
已按同一套流程收进令牌层：9 处字面量替换为 `var(--t-*)`，theme.css 增至 **464 个令牌**
（新增 `bg-14b8a6` / `ink-0f766e` / `ink-8a5b12` / `bd-9ad3cb` / `bd-f0d5a8`，另 4 个
色值与既有令牌重合直接复用）。`theme-tokens.py verify` 门禁通过：令牌闭合、浅色恒等、
暗色对比度全部 ≥3:1（文字类 ≥4.5:1，见 `preview` 审计表）；`workspace-smoke` 298/298。
注意：直接跑 `apply` 会用本次扫描结果重写整个 theme.css（把已有令牌冲掉）——增量收编
必须「扫描结果 ∪ theme.css 既有令牌」取并集后再落盘。
