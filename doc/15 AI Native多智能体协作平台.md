...
- **本地工作区（work_dir）**：用户选定一个工作文件夹后，Agent 的业务范围限定在该目录内——目录内可增删改查（默认只读，由用户提档），目录之外一律拒绝并记审计。Web UI 形态下「弹窗选文件夹」落地为「宿主固定根 + 根内子目录选择」，档位、路径守卫与删除/覆盖审批见 ADR-033
- **出网边界**：文件沙箱之外单独做一层网络策略——默认只允许访问公网地址，拦截 `127.0.0.1`、`10.x`、`172.16-31`、`192.168.x` 等内网网段与云元数据地址，支持域名白/黑名单；平台内部依赖与模型流量单列豁免，见 ADR-034
...
| 工具调用的安全性 | 三层边界叠加：代码执行走容器沙箱隔离（限制系统资源与网络权限）；文件访问限定在工作区内，目录之外一律拒绝（ADR-033）；出网再单独一层策略，默认拦内网、放公网并支持白/黑名单（ADR-034） |
