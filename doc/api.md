...
### 5.24 搜索渠道运行期配置（ADR-039）

`web_search` 走哪条出口、用哪份凭据，可以在「工具与配置 → 内部工具」里改，**不需要重建容器**。

| 方法 | 路径 | 用途 |
| --- | --- | --- |
| GET | `/api/v1/config/search` | 读生效值 + 环境基线 + 可选渠道目录 |
| PUT | `/api/v1/config/search` | 写覆盖值：字段缺省 = 不改动，显式 `null` = 清除该字段的覆盖 |

`GET` 响应字段：

| 字段 | 含义 |
| --- | --- |
| `provider` / `endpoint` | **生效值**（环境配置 ← 覆盖行中非空的字段） |
| `api_key_configured` | 是否已有可用凭据。**响应永不包含 `api_key` 原值** |
| `timeout_seconds` / `max_results` | 生效的检索参数（仍只由环境决定，不可运行期改） |
| `env_provider` / `env_endpoint` / `env_api_key_configured` | 纯环境基线，供界面回答「值从哪来」 |
| `overridden` | 三列里**是否有任何**覆盖值（见下第 2 条） |
| `updated_by` / `updated_at` | 最近一次写入；无覆盖时为 `null` |
| `channels[]` | 渠道目录：`id` / `label` / `needs_api_key` / `default_endpoint` / `description` |

两条容易踩错、且已在服务端固化的语义：

1. **换渠道时端点跟着换**。`endpoint` 只是地址，解析契约由 `provider` 决定。若 `provider` 改成
   与当前生效渠道不同的值、且本次调用没给端点（缺省 / `null` / 空串在换渠道时同义），服务端会
   替它落一个：环境渠道就是新渠道 ⇒ 用**环境端点**（保住本地网关）；否则 ⇒ 该渠道的
   **官方默认端点**。前端因此**换渠道时清空端点输入框**，不把旧渠道的端点当显式覆盖发出去。
2. **`overridden` 看的是三列，不是「行存在」**。清空覆盖后行仍在（三列皆 `null`），此时必须回
   `overridden=false`、且 `updated_by` / `updated_at` 一并回 `null`；否则界面会永久停在
   「已覆盖环境配置」，用户再也回不到「跟随环境配置」。

错误口径：输入不合法 → **422** `VALIDATION_ERROR`；事实源写入失败 → **503**
`DATA_SOURCE_UNAVAILABLE`。保存成功后服务端失效**两层**进程缓存（工具设置 + 工具注册表实例
缓存），因此下一次任务即生效、无需重启。

> 编号说明：本节取 **§5.24**。§5.22 已被两条并行线各占一份（长期记忆 / 运行期实时进度），
> §5.23 未落号，为不与在途小节撞号而顺延。
