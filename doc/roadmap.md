...
### 后续演进：工作区沙箱、破坏性动作审批与出网策略（2026-09-23，成员 D）

需求是「Agent 的业务限定在用户选定的工作文件夹内，目录内可增删改查、目录外禁止访问」，
外加一层独立的出网白/黑名单。设计先落两份 ADR
（[ADR-033](decisions/033-workspace-sandbox.md) / [ADR-034](decisions/034-egress-policy.md)），
再按四个阶段实现：

| 阶段 | 内容 | 关键取舍 |
| --- | --- | --- |
| 1 | 工作区登记 + 路径守卫 + 目录树 + 只读工具 | 授权单位是「宿主固定根 + 根内子目录」：浏览器给不了后端宿主路径，bind mount 又在容器创建时固定 |
| 2 | 写档位 + 三个**非破坏性**写工具 + 配额 + 提档接口 | 删除与覆盖要人工审批，所以先把它们挡住而不是先放开再说 |
| 3 | `approvals` 表与接口 + 软删除 + 覆盖/移动覆盖走审批 | **不做工作流级挂起**，改「批准一次、放行一次」（放行后置 `consumed`，比长期授权更强） |
| 4 | 沙箱挂载会话工作区 + 出网策略 | 沙箱是**兄弟容器**，bind 来源必须是宿主路径（从 mountinfo 反查）；策略校验放在打开会话时，保住 ADR-026 的「目录零 IO」 |

**本轮交付**：`app/workspace/`（守卫、服务、审批）、`app/security/`（判定 + 钉扎取数）、
`workspaces` / `approvals` 两张表、7 个工作区接口与 2 个审批接口（§5.19–§5.20）、
`GET /config/egress` 只读投影（§5.21）、六个工作区工具（读两个、写四个含审批）、
沙箱按档位挂 `rw`/`ro`。测试新增/改写 **225 例**，全量 902 passed。

**两个如实记录的取舍**：① `rw` 挂载下沙箱代码可以绕过审批直接写/删工作区文件——
要严格执行审批就设 `WORKSPACE_SANDBOX_MOUNT=ro`；② Docker Desktop 把 bind 统一显示为
uid 0，非 0 身份的沙箱写不进去，所以 compose 默认 `SANDBOX_UID/GID=0`。

**第二轮补齐（同日）**：ADR-034 原先列的四项已全部落地并实测——新增 `egress-proxy`
作为硬边界（明文 HTTP 绝对 URI + HTTPS `CONNECT`，共用同一套判定与钉扎），MCP 与模型
SDK 靠 `HTTP_PROXY`/`HTTPS_PROXY` 整段落到代理上（因此不再需要 SDK 内部钉扎），
search-gateway 脚本显式接入代理，沙箱联网时只接内部网络并注入代理变量；最后把
**backend 与 search-gateway 挪到 internal-only 网络**，从物理上断掉直连（实测：内网里
外部 DNS 都不通，但经代理的 HTTPS 返回 200；私网与云元数据由代理回 403）。
代价与连带改动写在 `doc/testing.md` §4.16：backend 不再发布 8000、宿主入口改走前端反代，
内网模型端点必须经代理放行。

**前端已接（同日）**：「工具与配置」新增「工作区」分区（登记 / 档位提降 / 目录树与配额），
「执行边界」扩成沙箱 + 出网策略（只读），对话流里加**审批卡片**（覆盖 / 删除由人放行一次，
一次一授权）。冒烟：`config-smoke` 86/86、`workspace-smoke` 210/210，`npm run build` 通过；
...
（详见 `doc/testing.md` §4.29）。边界在这里说清：

- **模型**：Token 采样带 `model` 标签，画布与执行台报的都是采样值，**已如实**；
- **Temperature / Top P / 输出上限 / 推理模式**：平台执行时**不落盘**，只有 Agent 目录里的
  当前值。界面不再冒充成「本次的参数」，而是归到「角色当前配置」并注明可能与当时不同。

**待完善（已定调，暂不做）**：这四项要精确回答「当时是多少」，需要执行时落一份**参数快照**——
在调用处把生效参数随采样一起写入（新表或扩展 `metrics` 标签，属 B/C 线的采集与数据模型改动，
前端只读）。当前结论：**模型记录正确即可**，其余项以后要做时再立项；在那之前，界面的
「角色当前配置」标注就是它们的准确说法，不算缺陷。