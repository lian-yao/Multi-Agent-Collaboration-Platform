...
| Web UI 与部署编排 | D | 已完成（浏览器渲染与依赖模型的部分除外） | `deploy/start.ps1` / `stop.ps1` 全流程实跑通过，并修复两处 Windows PowerShell 5.1 下被 `docker compose` stderr 中断的缺陷；E-04 的 Web 侧数据路径（经 nginx 反代）补了自动化用例；`doc/deployment.md` 补「演示与验收」与浏览器核对清单。详见 `doc/testing.md` §4.3 |
...
- **M5 状态**：E-01/E-02 的「结构化报告」曾因缺口 F-02 不达成，该缺口已在
  ...
  已于 2026-09-15 实跑通过。
  **M5 仍未闭环的部分**：`test_live_e2e.py` 里依赖模型的 4 条（E-01/E-02 两条、I-06、
  E-04 API 侧续跑）本轮未跑（本机无可用凭据）；Web UI 的浏览器渲染仍为人工核对；
...
- 2026-09-15（D 侧 D9-10：Web UI 与部署编排）：在本分支补 D 负责的两行。
  ...
  - 验证（本机 compose）：`uv run pytest -q -p no:cacheprovider` → **359 passed / 0 failed**，
    ...
    停止后项目容器全部移除。
  - **未完成**：依赖模型的 4 条 live 用例（E-01/E-02 两条、I-06、E-04 API 侧续跑）
    本轮未跑，本机无可用模型凭据；浏览器渲染仍为人工核对；「任务历史」的完整列表
    需要新增接口，本轮未做。
- 注意事项：数据库读取用例使用 SQLite 内存表与注入目录数据，MCP 用例走内存协议往返而非
  跨进程 stdio，因此不代表真实 PostgreSQL、真实 MCP Server 或浏览器端到端验收。
  E-04/E-05 的 Web 侧与部署脚本已在本轮补上实跑证据（见上一条），
  **浏览器端渲染与依赖模型链路的验收仍未完成**；全量 `uv run pytest` 隐含需要一个
  可达的 PostgreSQL 与 Redis。
