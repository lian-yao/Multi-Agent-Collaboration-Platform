...
| **让 Agent 自己读文件**（沙箱 + `read_file` 类工具） | **未做**，属 C 线 `app/sandbox`；需先修 docker.sock 或换无 Docker 的隔离后端 |
| 「已发送文档的原件下载」（当前只有图片留字节，§5.16.2） | **未做**，需先改存储策略 |
| 图片理解接真实视觉模型的回归 | **未做**，取决于提供方 |
| 附件的真实后端联调（容器重建 + 拖文件全链路） | **未做** |

这一轮把「模型能看见你传的文件」这条通路打通了，但它与「Agent 能自己去读文件」是两条不同的
路：前者不需要沙箱，现在就能用；后者要等 C 线把沙箱与文件工具补齐，两者可以并存而不是替代。
