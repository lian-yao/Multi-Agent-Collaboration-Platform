...
- **未解决、需要跟进的**：
  ...
  - **没有并行**：见上「决策 5」；
  - **规划质量已开始度量（D9-10 收尾），但仍没有降级率**：新增 `scripts/orchestration_ab.py`，
    对同一批任务分别跑静态与动态、对比耗时/模型调用数/输入输出 token/交付物字符与
    「必备内容是否命中」，报告落在 `doc/evals/orchestration-ab.md`。它评的是**净收益**
    （成本与产出一起看），不是计划本身的合理性；「计划是否比固定三步更贴任务」仍只能人工
    读交付物。另外 `tests/e2e/test_live_e2e.py` 是**走 HTTP 的部署环境回归**，仍未覆盖
    `orchestration_mode=dynamic` 的端到端链路。
