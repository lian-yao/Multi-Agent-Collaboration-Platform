...
- **未解决（明确接受的代价）**：`GET /api/v1/attachments/{id}/content` **只对图片回字节**。
  文本与文档的原始字节在上传时就丢掉了（只留抽取后的正文），因此**界面上无法下载已发送的
  文档原件**。这是「只有图片留字节」的直接代价；若将来需要「下载原件」，要先改存储策略
  （全部留字节，或有条件留），而不是只加一个按钮。
