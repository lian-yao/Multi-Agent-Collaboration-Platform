...
9. **密钥以明文存储在事实源与缓存中，访问边界由部署保证**（2026-09-15 用户确认）：
   静态加密不在本期范围；防线是数据库访问权限 + 部署网络边界，加上接口与日志的
   脱敏。后续如需静态加密，单独评估并另立 ADR。
...
- **前端入口（2026-09-15）**：工作台「工具与配置」页新增 Provider 配置表单
  （`frontend/src/Inspection.tsx`），`doc/api.md` §7 原先「写接口没有前端入口」的约束
  据此修订。同日按用户决定取消写入鉴权（ADR-015），因此页面**不再需要管理员令牌**：
  保存只发送配置字段本身，边界由部署网络承担。
