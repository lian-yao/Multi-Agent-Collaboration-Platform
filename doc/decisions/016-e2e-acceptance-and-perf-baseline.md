...
**新发现：F-07 阶段空 content 会静默降级下游**（C 侧上报，**已于 2026-09-20 处置**）。
...
**处置（方案 2，人类确认）**：空输出补一次「请用文字给出本阶段结论，不要再调用工具」的
重试，仍为空则落 WARNING 并继续；实现与实测见 ADR-009 的 2026-09-20 修订。
**根因更正与数据更正**：重新按「键对应阶段自身的 content」精确统计（先前的 10/18 是用正则
扫整份载荷、会把嵌套的上游字段一并算进去，偏高）：全量 21 个工作流共 63 份阶段载荷中
**14 份（22%）为空**，大多数工作流只空 1 个阶段。真因不是「模型给了空回复」，而是
**模型撞上工具轮次上限（4）后仍在请求工具**：日志显示 collect/analyze 常有 7–9 次工具调用、
`pending_tool_calls=2..4`，阶段因此没有文字产出；首版兜底把「还有待处理 tool_calls」当作
有产出，导致补提示不触发（实测仍 8/12 为空），修正判空后才生效。
处置后的实测：同批 4 个工作流 12 份阶段载荷 **0 份为空**，日志 7 次
`stage.tool_iteration_limit` + 5 次 `stage.empty_content action=retry`，无
`continue_with_empty`（5/5 救回）；代价是 live 套件耗时 168.87s → 343.69s。
