...
失败重试**不新增行**：单次逻辑调用失败后按 `TOOL_CALL_RETRY_LIMIT = 3` 重试（最多 4 次尝试），
...
索引：`ux_workspaces_session (session_id)`（唯一）——含义是「**一个会话只绑定一个工作区**」。

**一个会话 = 一个工作区**（2026-09-23 修正之二）：**再登记就是改绑**，旧绑定在同一步里解除，
不会留下第二条。这不是收紧自由度，而是修一个真缺陷——执行侧按 `session_id` 取工作区时只能取
一条，之前"多条登记 + 取最早那条"会让「界面上选了哪个」与「Agent 实际用哪个」不一致
（实测：同一会话登记 `D:\测试` 与 `C:\Users\zq\MacpWorkspace` 后，Agent 用的是先登记的
`D:\测试`）。使用者的原始要求也是**一个**文件夹：「选一个文件夹……也只能在这个文件夹下面」。

**不同会话可以各自绑定同一个文件夹**（2026-09-23 修正之一）：一个目录被多个项目共用是常态，
全局唯一会让第二个项目直接 409。各会话持有自己的档位与配额，互不影响；`.trash`（软删除目录）
落在同一个物理目录里，这是共用的代价，不是缺陷；审批按 `workspace_id` 匹配，所以 A 会话批准的
覆盖/删除**不会**顺带放行 B 会话。
