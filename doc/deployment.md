...
| `TOOL_SEARCH_PROVIDER` | 搜索出口实现：`duckduckgo`（默认，DuckDuckGo / 网关契约）/ `volcengine`（火山引擎豆包搜索 Custom，POST + Bearer key，ADR-037）。选 `volcengine` 且不写 `TOOL_SEARCH_ENDPOINT` 时端点自动取 `https://open.feedcoopapi.com/search_api/web_search` |
| `TOOL_SEARCH_API_KEY` | `TOOL_SEARCH_PROVIDER=volcengine` 时的按量付费 API key。只从环境 / `.env` 读（`.env` 已被 gitignore），**不进仓库、不进日志**；留空时工具直接报配置错，不发出无鉴权请求 |
